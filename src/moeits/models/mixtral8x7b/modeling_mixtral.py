...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        if config.num_experts_by_block:
            self.gate = nn.Linear(self.hidden_dim, self.config.num_experts_by_block[self.layer_idx], bias=False)
            self.experts = nn.ModuleList([MixtralBlockSparseTop2MLP(config) for _ in range(self.config.num_experts_by_block[self.layer_idx])])
        else:
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.num_experts_by_block:
            expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.config.num_experts_by_block[self.layer_idx]).permute(2, 1, 0)
        else:
            expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0)

        # Loop over all available experts in the model and perform the computation on each expert
        if self.config.num_experts_by_block:
            for expert_idx in range(self.config.num_experts_by_block[self.layer_idx]):
