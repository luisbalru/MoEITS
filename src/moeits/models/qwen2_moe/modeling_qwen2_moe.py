...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if config.num_experts_by_block:
            # gating
            self.gate = nn.Linear(config.hidden_size, config.num_experts_by_block[layer_idx], bias=False)
            self.experts = nn.ModuleList(
                [Qwen2MoeMLP(config, intermediate_size=config.moe_intermediate_size) for _ in range(config.num_experts_by_block[layer_idx])]
            )
        else:
            # gating
