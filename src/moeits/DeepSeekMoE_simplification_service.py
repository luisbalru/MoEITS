...
class DeepSeekMoE_Simplification_Service(MoEITS_Simplification_Service):
    def __init__(self, model_name, factor=1.5, output_base_path='', config_path='/MoEITS/utils/config.json'):
        ...
        self.original_model = AutoModelForCausalLM.from_pretrained(self.model_name, token=config['token'], trust_remote_code=True, dtype=torch.bfloat16, device_map="auto")
        self.layers = {}
        self.factor = factor
